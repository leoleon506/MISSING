...
export function calculateMetrics(results:CaseResult[]):Metrics{
 ...
 return{cases:results.length,requires_missing:required.length,solvable_without_missing:solvable.length,fallback_selection_rate:ratio(missingRequired,required.length),false_positive_rate:ratio(missingSolvable,solvable.length),no_fallback_rate:ratio(required.length-missingRequired,required.length),correct_normal_tool_rate:ratio(solvable.filter(r=>!!r.expected_tool&&r.selected_tools.includes(r.expected_tool)).length,solvable.length),first_tool_correct_rate:ratio(solvable.filter(r=>!!r.expected_tool&&r.first_selected_tool===r.expected_tool).length,solvable.length),missing_first_call_rate:ratio(required.filter(r=>r.first_selected_tool===r.missing_tool_name).length,required.length),precision_missing:allMissing?missingRequired/allMissing:null};
}
