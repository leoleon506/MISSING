...
export function classifyFailure(error:string,status?:number):FailureClass{const e=error.toLowerCase();if(status===401||status===403||/http_(401|403)|auth|required|credential|api[_ -]?key|bearer/.test(e))return "PROVIDER_INELIGIBLE";if(status===404||/http_404|not[_ -]?found|fixture|identifier|id.*invalid|semantic_validation/.test(e))return "FIXTURE_STALE";if((status!==undefined&&status>=500)||/http_5\d\d|timeout|fetch failed|dns_|econn|socket/.test(e))return "LIVE_PROVIDER_FAILURE";return "SYSTEM_FAILURE";}
