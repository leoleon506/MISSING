...
export async function verifyJsonRequest(url:string,maxBytes:number){
  const first=await oneGet(url,maxBytes),trace:any={initial_url:url,initial_status:first.status,redirect_used:false,redirect_target:null,final_status:first.status};let final=first,finalUrl=url;
