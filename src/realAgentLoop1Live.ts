  ...
  try {
    const result = await pool.query(`
      SELECT payment_hash, request_hash, execution_id, capability, state,
             created_at, updated_at, transaction_reference,
...
async function main() {
  ...
  const evaluation = evaluateRealAgentPaidClosedLoop({ demands, promotions, payments });
  const evidence = {
    result: evaluation.qualified ? "GO_REAL_AGENT_PAID_CLOSED_LOOP" : "NO_GO_REAL_AGENT_PAID_CLOSED_LOOP",
    proof_boundary: "production_demand_supply_ledgers_and_shared_postgres",
