...
if(!integrity.ok){
  const invalid={experiment:"5A",decision:"INVALID_5A_ENGINE_INTEGRITY",integrity};
  await writeFile(`${outDir}/report.json`,JSON.stringify(invalid,null,2));
  throw new Error(`5a_engine_integrity_failed:${integrity.errors.join("|")}`);
}
...
try{
  ...
  execFileSync(process.execPath,["--import","tsx","src/experiment4w.ts"],{
    cwd:repoRoot,
    env:process.env,
    stdio:"inherit"
  });
...
if(engineError){
  const failure={experiment:"5A",decision:"REASSESS_5A_BLIND_GENERALIZATION_HOLDOUT",integrity,harness_error:String(engineError?.stack||engineError)};
  await writeFile(`${outDir}/harness-failure.json`,JSON.stringify(failure,null,2));
  throw engineError;
}
