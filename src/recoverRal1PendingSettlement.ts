...
const ALLOWED_REASONS = new Set([
  "settlement_pending",
  "settlement_transaction_observed",
  "settlement_proof_pending:receipt_block_ahead_of_head",
  "settlement_proof_pending:insufficient_confirmations",
]);
...
async function main() {
  if (process.env.NODE_ENV !== "production") throw new Error("RAL1 pending settlement recovery requires NODE_ENV=production");
  if (process.env.MISSING_RAL1_RECOVER_PENDING_SETTLEMENT !== "1") {
    throw new Error("RAL1 pending settlement recovery requires MISSING_RAL1_RECOVER_PENDING_SETTLEMENT=1");
  }
  ...
  const pool = new Pool({
    ...
    application_name: "missing-ral1-pending-settlement-recovery",
  });
  ...
  try {
    ...
    if (row.state !== "settling") throw new Error(`recovery refuses non-settling state: ${row.state}`);
    if (!ALLOWED_REASONS.has(String(row.reason ?? ""))) throw new Error(`recovery refuses settlement reason: ${row.reason}`);
    ...
    try {
      ...
      `, [row.payment_hash, row.execution_id, row.reason, transaction]);
      if (updated.rowCount !== 1) throw new Error("durable payment changed during recovery; no mutation committed");
      await client.query("COMMIT");
      console.log(JSON.stringify({
        recovered: true,
        recovery_kind: "exact_onchain_reproof_of_known_pending_settlement",
        transaction,
        proof,
