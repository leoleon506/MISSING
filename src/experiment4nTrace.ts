...
export type ExecutedGetTrace4N={
  trace_id:string;
  case_id:string;
  provider_candidate_id:string;
  stage:ExecutedTraceStage4N;
  parent_evidence_ids:string[];
  requested_url:string;
  final_url:string|null;
  requested_at:string;
  disposition:ExecutedTraceDisposition4N;
  content_type:string|null;
  body_fingerprint:string|null;
  body_bytes:number;
  response_text:string;
  network_origin:ExecutedTraceOrigin4N;
  error:string|null;
};
...
let seq=0;
let CURRENT:{case_id:string;provider_candidate_id:string}|null=null;
...
function pushTrace(ctx:TraceConsumerContext4N,args:{requested_url:string;final_url:string|null;requested_at:string;disposition:ExecutedTraceDisposition4N;content_type:string|null;text:string;network_origin:ExecutedTraceOrigin4N;error:string|null}){
  const trace:ExecutedGetTrace4N={trace_id:traceId(ctx,args.requested_url,args.requested_at),case_id:ctx.case_id,provider_candidate_id:ctx.provider_candidate_id,stage:ctx.stage,parent_evidence_ids:[...new Set(ctx.parent_evidence_ids)],requested_url:stableUrl(args.requested_url),final_url:args.final_url?stableUrl(args.final_url):null,requested_at:args.requested_at,disposition:args.disposition,content_type:args.content_type,body_fingerprint:args.disposition==="success"?sha3yr(args.text):null,body_bytes:args.disposition==="success"?Buffer.byteLength(args.text):0,response_text:args.disposition==="success"?args.text:"",network_origin:args.network_origin,error:args.error};
  TRACES.push(trace);
  return trace;
}
...
async function tracedCached(kind:"doc"|"spec",ctx:TraceConsumerContext4N,startUrl:string,url:string,maxBytes:number,ledger:RecoveryLedger,fetcher?:Fetcher){
  const requested_at=new Date().toISOString(),before=acquisitionCacheMetrics4M();
  try{
    const got=kind==="doc"?await fetchDocTextSafe4M(startUrl,url,maxBytes,ledger,fetcher):await fetchSpecTextSafe4M(startUrl,url,maxBytes,ledger,fetcher),after=acquisitionCacheMetrics4M(),origin=cacheOrigin(before,after,kind);
    pushTrace(ctx,{requested_url:url,final_url:got.final_url,requested_at,disposition:"success",content_type:got.content_type,text:got.text,network_origin:origin,error:null});
    return got;
  }catch(e){
    const after=acquisitionCacheMetrics4M(),origin=cacheOrigin(before,after,kind),error=String(e).replace(/^Error:\s*/,"");
    pushTrace(ctx,{requested_url:url,final_url:null,requested_at,disposition:"error",content_type:null,text:"",network_origin:origin,error});
    throw e;
  }
}
...
export function recordReferenceExpansionTrace4N(args:{parent_evidence_id:string;requested_url:string;final_url:string|null;requested_at:string;status:number;content_type:string|null;text:string;error?:string|null}){
  if(!CURRENT)return null;
  const ctx:TraceConsumerContext4N={case_id:CURRENT.case_id,provider_candidate_id:CURRENT.provider_candidate_id,stage:"REFERENCE_EXPANSION",parent_evidence_ids:[args.parent_evidence_id]};
  const ok=args.status>=200&&args.status<300&&!!args.text;
  return pushTrace(ctx,{requested_url:args.requested_url,final_url:args.final_url,requested_at:args.requested_at,disposition:ok?"success":"error",content_type:args.content_type,text:ok?args.text:"",network_origin:"network",error:ok?null:(args.error||`http_${args.status||0}`)});
}
