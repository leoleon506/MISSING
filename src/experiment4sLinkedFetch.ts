...
import {countLinkedAuthOperations4S,extractLinkedContractCandidates4S,linkedSpecSummary4S} from "./experiment4sLinked.js";

export async function ingestLinkedContracts4S(evidence:DocEvidence[],provider:Provider4A,ledger:RecoveryLedger){
  const candidates=extractLinkedContractCandidates4S(evidence,provider.start_url),added:DocEvidence[]=[];let fetches=0,fetchSuccesses=0,parseSuccesses=0,rejectedCrossProvider=0,rejectedAuth=0,operations=0;
  let seq=0;
  for(const c of candidates.slice(0,6)){
    fetches++;
    try{
      const got=await fetchTextSafeR2(provider.start_url,c.target_url,FOUR_A_BUDGET.max_bytes,ledger);fetchSuccesses++;
      const spec=parseOpenApiText(got.text);if(!spec)continue;parseSuccesses++;rejectedAuth+=countLinkedAuthOperations4S(spec);
  ...
  return {evidence:added,metrics:{linkedContractLinksDiscovered:candidates.length,linkedContractFetches:fetches,linkedContractFetchSuccesses:fetchSuccesses,linkedContractParseSuccesses:parseSuccesses,linkedContractRejectedCrossProvider:rejectedCrossProvider,linkedContractRejectedAuth:rejectedAuth,linkedContractOperationsDiscovered:operations},linked_evidence_ids:added.map(e=>e.evidence_id)};
}
