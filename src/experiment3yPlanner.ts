...
function plainText(text:string){if(!/<[a-z!/][\s\S]*>/i.test(text))return text;return text.replace(/<script\b[^>]*>[\s\S]*?<\/script>/gi," ").replace(/<style\b[^>]*>[\s\S]*?<\/style>/gi," ").replace(/<svg\b[^>]*>[\s\S]*?<\/svg>/gi," ").replace(/<br\s*\/?>/gi,"\n").replace(/<\/p>|<\/div>|<\/li>|<\/h[1-6]>/gi,"\n").replace(/<[^>]+>/g," ").replace(/&quot;/g,'"').replace(/&#39;|&apos;/g,"'").replace(/&amp;/g,"&").replace(/&lt;/g,"<").replace(/&gt;/g,">").replace(/&nbsp;/g," ").replace(/&#(\d+);/g,(_,n)=>String.fromCharCode(Number(n))).replace(/[ \t]+/g," ").replace(/\n\s*\n+/g,"\n").trim();}
function terms(c:HoldoutCase){return [...new Set(["GET","https://","endpoint","example response","response","authentication","authorization","api key","no auth","without authentication",...c.input_names,...c.required_output_leaf_names,...c.intent.toLowerCase().match(/[a-z0-9_]+/g)||[]].filter(x=>String(x).length>=3).map(x=>String(x).toLowerCase()))];}
function compact(raw:string,c:HoldoutCase,max=9000){const text=plainText(raw);if(text.length<=max)return text;const low=text.toLowerCase(),ranges:{a:number;b:number}[]=[];for(const t of terms(c)){let from=0,hits=0;while(hits<6){const i=low.indexOf(t,from);if(i<0)break;ranges.push({a:Math.max(0,i-900),b:Math.min(text.length,i+2100)});from=i+t.length;hits++;}}ranges.sort((x,y)=>x.a-y.a);const merged:{a:number;b:number}[]=[];for(const r of ranges){const last=merged.at(-1);if(last&&r.a<=last.b+80)last.b=Math.max(last.b,r.b);else merged.push({...r});}let out="";for(const r of merged){const chunk=text.slice(r.a,r.b);if(out.length+chunk.length>max)out+=chunk.slice(0,Math.max(0,max-out.length));else out+=chunk;if(out.length>=max)break;out+="\n...[relevance window]...\n";}return out||text.slice(0,max);}
function evidencePayload(evidence:DocEvidence[],c:HoldoutCase){const ok=evidence.filter(e=>e.state==="ok"),per=Math.max(6500,Math.floor(56000/Math.max(1,ok.length)));return ok.map(e=>({evidence_id:e.evidence_id,url:e.resolved_url,content_type:e.content_type,text:compact(e.text,c,Math.min(12000,per))})).filter(x=>x.text.length>0);}
...
export async function compileCapability(config:Config,c:HoldoutCase,p:ProviderRef,evidence:DocEvidence[],phase:"initial"|"repair",previousError?:string){const system=[
 ...
 "Return one JSON object only. If the evidence does not support a public GET satisfying every required output, return REJECT.",
 "A public documented GET may be compiled without credentials when the supplied documentation does not require authentication for that endpoint. Never invent or send credentials, Authorization headers, API keys, cookies, or other auth material.",
