...
async function callPlanner(config:ReturnType<typeof loadConfig>,c:Case,sources:Source[],failed?:{recipe:Recipe;error:string}){
  ...
  const response=await fetch(`${config.baseUrl.replace(/\/$/,"")}/chat/completions`,{method:"POST",headers:{authorization:`Bearer ${config.apiKey}`,"content-type":"application/json"},body:JSON.stringify({model:config.model,temperature:0,messages:[{role:"system",content:system},{role:"user",content:JSON.stringify(payload)}],response_format:{type:"json_object"}})});if(!response.ok)throw new Error(`planner ${response.status}: ${await response.text()}`);const body:any=await response.json();const text=body.choices?.[0]?.message?.content;if(!text)throw new Error("planner returned no JSON");return {recipe:JSON.parse(text) as Recipe,usage:body.usage??null};
}
