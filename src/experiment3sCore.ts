...
export type RedirectDecision={ok:boolean;reason:string|null;target_url:string|null;canonical_host:string|null};
...
export function validateRedirect(originalUrl:string,location:string,redirectCount:number):RedirectDecision{
  if(redirectCount>=1)return {ok:false,reason:"redirect_chain_rejected",target_url:null,canonical_host:null};
  let original:URL,target:URL;try{original=new URL(originalUrl);target=new URL(location,original);}catch{return {ok:false,reason:"redirect_invalid_location",target_url:null,canonical_host:null};}
  if(target.protocol!=="https:")return {ok:false,reason:"redirect_non_https",target_url:null,canonical_host:null};
  if(isIP(target.hostname))return {ok:false,reason:"redirect_ip_literal",target_url:null,canonical_host:null};
  if(normalizePathQuery(original)!==normalizePathQuery(target))return {ok:false,reason:"redirect_path_changed",target_url:null,canonical_host:null};
  return {ok:true,reason:null,target_url:target.toString(),canonical_host:target.hostname};
}

export function canonicalBaseFromRedirect(targetUrl:string,operationPath:string){
  const u=new URL(targetUrl),pathWithoutQuery=u.pathname;if(!pathWithoutQuery.endsWith(operationPath))throw new Error("redirect_operation_path_suffix_mismatch");const prefix=pathWithoutQuery.slice(0,pathWithoutQuery.length-operationPath.length);return `${u.protocol}//${u.host}${prefix}`.replace(/\/$/,"");
}
export function verificationTraceFingerprint(trace:any){return createHash("sha256").update(JSON.stringify(trace)).digest("hex");}
...
export function runNegativeControls():CoreControl[]{
  const controls=[...run3rControls()];const base="https://api.example.test/v1/items?id=1";
  ...
  d=validateRedirect(base,"https://api2.example.test/v1/items?id=1",1);controls.push({control:"second_redirect",executed:true,rejected:!d.ok&&d.reason==="redirect_chain_rejected",evidence:d});return controls;
}
