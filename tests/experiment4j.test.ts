...
describe("4J variable role and entity compatibility",()=>{
 it("rejects book operation for character task",async()=>{const g=await prepareRequestGraph4J([ev('<h2>Books</h2><p>Get a book by id</p><code>GET https://api.example.test/books/{id}</code>')],"fictional_character_metadata",new RecoveryLedger());expect(g.hypotheses.some(h=>h.full_path.includes("/books/"))).toBe(false)});
 it("accepts character generic id with character context",async()=>{const g=await prepareRequestGraph4J([ev('<h2>Characters</h2><p>Get a fictional character by id</p><code>GET https://api.example.test/characters/{id}</code>')],"fictional_character_metadata",new RecoveryLedger());expect(g.hypotheses.some(h=>h.full_path.includes("/characters/{id}"))).toBe(true)});
 it("accepts VIN placeholder without build-value example",async()=>{const g=await prepareRequestGraph4J([ev('<h2>Vehicle VIN Decoder</h2><p>Decode a vehicle identification number.</p><code>GET https://api.example.test/vehicles/decode/{vin}</code>')],"vin_vehicle_metadata",new RecoveryLedger());expect(g.hypotheses.some(h=>h.full_path.includes("{vin}"))).toBe(true)});
 it("accepts contextual product code",async()=>{const g=await prepareRequestGraph4J([ev('<h2>Product lookup</h2><p>Lookup retail food product information by code.</p><code>GET https://api.example.test/products/{code}</code>')],"food_barcode_metadata",new RecoveryLedger());expect(g.hypotheses.some(h=>h.full_path.includes("{code}"))).toBe(true)});
 it("keeps auth-like requests out of packet",async()=>{const g=await prepareRequestGraph4J([ev('<h2>Product lookup</h2><code>GET https://api.example.test/products/{code}?api_key={key}</code>')],"food_barcode_metadata",new RecoveryLedger());expect(g.metrics.authLikeRenderedRequests).toBe(0);expect(g.probe_packet.every(h=>!h.full_path.includes("api_key"))).toBe(true)});
