...
describe("Experiment 5B7 entity-coupled query operation gate",()=>{
 ...
 it("does not allow identity-role compatibility alone without entity evidence",()=>{const text=`<h2>Generic identifier lookup</h2><p>GET https://api.example.test/lookup?id=example</p><p>id: identifier value</p><pre>{"name":"x","version":"1"}</pre>`,r=buildGatedQueryHypotheses5B7([ev(text)],provider);expect(r.hypotheses).toHaveLength(0);expect(r.metrics.queryEntityCouplingRejects5b7).toBeGreaterThan(0)});
