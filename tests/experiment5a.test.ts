...
describe("Experiment 5A preregistered holdout",()=>{
 it("contains exactly 18 novel cases across 10 families",()=>{expect(FIVE_A_CASES).toHaveLength(18);expect(new Set(FIVE_A_CASES.map(c=>c.family)).size).toBe(10);expect(new Set(FIVE_A_CASES.map(c=>c.case_id)).size).toBe(18);const dev=new Set(DEVELOPMENT_CASES.map(c=>c.case_id));expect(FIVE_A_CASES.filter(c=>dev.has(c.case_id))).toEqual([])});
