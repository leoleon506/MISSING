...
describe("Experiment 3V",()=>{
  ...
  it("rejects unknown duplicates extras and too many selections",()=>{const broad:any[]=[{candidate_id:"p1"},{candidate_id:"p2"}];expect(validateRerankObject({case_id:"x",selected:[{candidate_id:"x",reason:""}]},broad).error).toContain("unknown_candidate_id");expect(validateRerankObject({case_id:"x",selected:[{candidate_id:"p1",reason:""},{candidate_id:"p1",reason:""}]},broad).error).toContain("duplicate_candidate_id");expect(validateRerankObject({case_id:"x",selected:[],url:"https://x"},broad).error).toContain("extra:url");});
