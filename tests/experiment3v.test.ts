...
describe("Experiment 3V",()=>{
  ...
  it("binds planner evidence to the requested case identity",()=>{const broad:any[]=[{candidate_id:"p1"}];const attempt:any={http_ok:true,http_status:200,refusal:null,parsed_json:{case_id:"wrong",selected:[{candidate_id:"p1",reason:"fit"}]},expected_case_id:"expected"};expect(parseRerankAttempt(attempt,broad).error).toContain("case_id_mismatch");});
