...
describe("4E DOM-native request evidence",()=>{
  ...
  it("separates multiple requests in one pre block",()=>{const r=extractOperations4E([ev('<pre>GET https://api.example.com/v1/a/{name}\nGET https://api.example.com/v1/b/{name}</pre>')]);expect(r.operations.length).toBe(2)});
