...
describe("4A-R3 typed binding IR",()=>{
 ...
 it("canonicalizes typed query INPUT and LITERAL without aliases",()=>{const r=base();r.query_bindings=[{name:"q",source_kind:"INPUT",input_name:"name",literal_value:null} as any,{name:"format",source_kind:"LITERAL",input_name:null,literal_value:"json"} as any];expect(canonicalizeBoundary3(r,["value"],["name"])).toMatchObject({query_bindings:{q:"$input.name",format:"json"}});});
 it("rejects a binding to an input not present in the case",()=>{const r=base();r.path_bindings=[{name:"name",source_kind:"INPUT",input_name:"other",literal_value:null} as any];expect(()=>canonicalizeBoundary3(r,["value"],["name"])).toThrow(/binding_input_invalid/);});
 it("rejects literal path bindings",()=>{const r=base();r.path_bindings=[{name:"name",source_kind:"LITERAL",input_name:null,literal_value:"x"} as any];expect(()=>canonicalizeBoundary3(r,["value"],["name"])).toThrow(/binding_source_kind_invalid/);});
 ...
 it("rejects INPUT bindings carrying a literal",()=>{const r=base();r.query_bindings=[{name:"q",source_kind:"INPUT",input_name:"name",literal_value:"name"} as any];expect(()=>canonicalizeBoundary3(r,["value"],["name"])).toThrow(/binding_input_literal_must_be_null/);});
});
