...
describe("MISSING Product Theta.2 OpenAPI recipe compiler", () => {
  ...
  it("derives deterministic path bindings and response projection", async () => {
    ...
    expect(result.candidate?.projection).toEqual({
      country: { op: "FIELD", path: "country" },
      normalized: { op: "FIELD", path: "normalized" },
  ...
  it("produces a Theta-ready candidate when two independent replay inputs are supplied", async () => {
    const result = await compileOpenApiLead(lead(), {
      ...
      family: "compliance",
      verificationInputs: [{ vat_number: "FI12345678" }, { vat_number: "FI87654321" }],
    });
  ...
  it("supports Swagger 2 HTTPS host and basePath", async () => {
      ...
      paths: {
        "/vat/{vat_number}": {
          get: {
            ...
            summary: "Finnish VAT validation",
            parameters: [{ name: "vat_number", in: "path", required: true, type: "string" }],
            responses: {
              "200": { schema: { type: "object", properties: { valid: { type: "boolean" }, country: { type: "string" } } } },
            },
          },
        },
      },
    ...
    const result = await compileOpenApiLead(lead(), {
      fetchFn: async () => new Response(JSON.stringify(swagger), { status: 200 }),
      verificationInputs: [{ vat_number: "FI1" }, { vat_number: "FI2" }],
    });
      ...
      paths: {
        "/vat/{vat_number}": {
          get: {
            ...
            parameters: [{ name: "vat_number", in: "path", required: true }],
